...
impl GraphApp {
    ...
    fn ensure_layout(&mut self, rect: Rect) {
        ...
        // Golden-angle spiral layout for any nodes that don't yet have positions
        let center = rect.center();
        // collect ids missing positions in a stable order
        ...
        // start index at current number of positioned nodes to keep spacing consistent
        let mut k = self.node_positions.len();
        for id in missing {
            let pos = golden_spiral_position(center, k as u32, rect);
...
impl eframe::App for GraphApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::TopBottomPanel::top("top_bar").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ui.label("Graph Viewer");
                ui.separator();
                ui.menu_button("File", |ui| {
                    if ui.button("Save").clicked() {
                        self.save_now(); ui.close_menu();
                    }
                    if ui.button("Save Versioned Copy").clicked() {
                        self.save_versioned_now(); ui.close_menu();
                    }
                    ...
                    if ui.button("Load Version…").clicked() { self.show_load_versions = true; ui.close_menu(); }
                });
                ...
                // Saved/loaded info is now shown as a transient toast at bottom-right
                if let Some(err) = &self.save_error { ui.separator(); ui.colored_label(Color32::RED, err); }
            });

            // No creation tooling in top bar anymore.
        });
        ...
        if self.sidebar_open && self.sidebar_mode == SidebarMode::Tooling {
            egui::SidePanel::left("tooling_sidebar")
                ...
                .show(ctx, |ui| {
                    ...
                    ui.collapsing("Optional: Pre-link a relationship", |ui| {
                        ui.horizontal(|ui| {
                    ...
                    if ui.button("Create Node").clicked() {
                        ...
                        if label.is_empty() {
                            ...
                            // Optionally create relationship involving the new node
                                ...
                                if let Some(other) = self.create_node_rel_target {
                                    if other != id {
                                        match self.create_node_rel_direction {
                                            NewNodeRelDir::NewToExisting => { let _ = self.db.add_relationship(id, other, rel_label.clone(), HashMap::new()); }
                                            NewNodeRelDir::ExistingToNew => { let _ = self.db.add_relationship(other, id, rel_label.clone(), HashMap::new()); }
                                        }
                                    }
                                } else {
                    ...
                    let mut btn = ui.add_enabled(!disabled, egui::Button::new("Apply"));
                    if btn.clicked() {
                    ...
                    let mut btn = ui.add_enabled(!disabled, egui::Button::new("Delete Keys"));
                    if btn.clicked() {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            // remember canvas rect for new-node placement
            ...
            // Zoom with scroll
            if ui.input(|i| i.raw_scroll_delta.y).abs() > 0.0 {
            ...
            for (id, node) in &self.db.nodes {
                ...
                // Dragging updates world position inverse-transforming pan/zoom delta
                if resp.dragged() && resp.drag_delta() != Vec2::ZERO {
                    // Convert screen delta back to world delta
                    let delta_screen = resp.drag_delta();
                    let delta_world = Vec2::new(delta_screen.x / self.zoom, delta_screen.y / self.zoom);
                    let entry = self.node_positions.get_mut(id).unwrap();
                    entry.x += delta_world.x;
                    entry.y += delta_world.y;
            ...
            let spring_k = 4.0_f32;      // spring stiffness (units/s^2)
                ...
                if let (Some(pa), Some(pb)) = (pa_opt, pb_opt) {
                    ...
                    if dist2 > 1e-6 {
                        ...
                        if stretch > 0.0 {
                            ...
                            if self.dragging != Some(a_id) {
                                *forces.entry(a_id).or_insert(Vec2::ZERO) += f;
                            }
                            if self.dragging != Some(b_id) {
            ...
            for i in 0..ids.len() {
                for j in (i + 1)..ids.len() {
                    ...
                    if dist < min_sep {
                        ...
                        // push opposite directions; dragged node receives no force
                        if self.dragging != Some(a) { *forces.entry(a).or_insert(Vec2::ZERO) -= f; }
                        if self.dragging != Some(b) { *forces.entry(b).or_insert(Vec2::ZERO) += f; }
                    }
                }
            }
            ...
            for (id, pos) in self.node_positions.clone() {
                // Skip nodes without positions (shouldn't happen) or when dragged (velocity zeroed)
                let dragging_this = self.dragging == Some(id);
                let mut v = *self.node_velocities.entry(id).or_insert(Vec2::ZERO);
                if dragging_this { v = Vec2::ZERO; }
                ...
                if dragging_this { step = Vec2::ZERO; }
                if step != Vec2::ZERO {
                ...
                // store updated velocity (zero if dragging)
                if dragging_this { v = Vec2::ZERO; }
        ...
        for id in open_node_ids {
            ...
            if let Some(node_snapshot) = node_snapshot {
                ...
                // Apply actions
                if do_save_label {
                    if self.db.update_node_label(id, label_text.clone()) { self.mark_dirty(); }
                }
                if !to_remove_keys.is_empty() {
                    for k in to_remove_keys { if self.db.remove_node_metadata_key(id, &k) { self.mark_dirty(); } }
                }
                if let Some((k, v)) = upsert_kv { if self.db.upsert_node_metadata(id, k, v) { self.mark_dirty(); } }
                // persist editors
        ...
        for rid in open_rel_ids {
            ...
            if let Some(rel_snapshot) = rel_snapshot {
                ...
                if save_label { if self.db.update_relationship_label(rid, label_text.clone()) { self.mark_dirty(); } }
                for k in remove_keys { if self.db.remove_relationship_metadata_key(rid, &k) { self.mark_dirty(); } }
                if let Some((k, v)) = upsert_rel_kv { if self.db.upsert_relationship_metadata(rid, k, v) { self.mark_dirty(); } }
                ...
                egui::Area::new("bottom_right_toast".into())
                    ...
                    .show(ctx, |ui| {
                        ...
                        egui::Frame::popup(ui.style())
                            ...
                            .stroke(egui::Stroke { width: stroke_w, color: stroke_col })
                            .fill(fill)
